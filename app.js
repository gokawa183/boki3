...
async function init(){
  // まず全タブを確実にhiddenにする
  ['quiz','stats','study','exam','mock','q2study','q3study','kessan','reference']
    .forEach(t => {
      const el = document.getElementById('tab-'+t);
      if(el) el.classList.add('hidden');
    });

  // 問題・チップは同期データのみ依存するため、ストレージ待機前に初期化
...
function showTab(tab,el){
  ['quiz','stats','study','exam','mock','q2study','q3study','kessan','reference','ledger','release'].forEach(t=>document.getElementById('tab-'+t).classList.add('hidden'));
...
const STUDY_LK_ORDER=['basics','shiwake','kamoku','bspl','deposit','cashcontent','koguchi','kakeuri','kashidaore','maekin','kitte','tegata','yuka','kotei','mibarai','maebara','kessanuri','hojinzei','soukanjo','hojobo','chosahyo','seizan'];
